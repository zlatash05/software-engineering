...
class Car:  - Объявление класса Car
    def __init__(self, make, model):  # Конструктор класса с параметрами make и model
        self.make = make      # Создание атрибута make для объекта
        self.model = model    # Создание атрибута model для объекта
...
`print(f"Driving the {self.make} {self.model}")`- выводит сообщение с использованием марки и модели
...
`def __init__(self, make, model, battery_capacity): super().__init__(make, model)` - вызов конструктора родительского класса
`my_electric_car = ElectricCar("Tesla", "Model S", 75)`  - Создание объекта электромобиля
